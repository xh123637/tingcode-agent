...
export function createDiscordConnection(
  ...
  async function handleMessage(
    ...
    try {
      ...
      // TinyCode (the previous implementation incorrectly keyed them by the
    ...
    async connect(opts: DiscordConnectOpts): Promise<boolean> {
      ...
      try {
        ...
        discordClient.once(Events.ClientReady, async (readyClient) => {
          ...
          // Register TinyCode slash commands as Discord Application Commands
          try {
        ...
        discordClient.on(Events.InteractionCreate, async (interaction) => {
          ...
          // Build command string matching TinyCode's text command format.
