...
bugReportRoutes.post('/submit', authMiddleware, async (c) => {
  ...
  const fullBody = `${body}\n\n---\n> Submitted via TinyCode by ${user.display_name || user.username}`;
  ...
  const url = `https://github.com/tinycode/tinycode/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(truncatedBody)}`;
