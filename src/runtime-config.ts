...
const DANGEROUS_ENV_VARS = new Set([
  ...
  'TMP',
  // TinyCode 内部路径映射
  'TINYCODE_WORKSPACE_GROUP',
  'TINYCODE_WORKSPACE_GLOBAL',
  'TINYCODE_WORKSPACE_MEMORY',
  'TINYCODE_WORKSPACE_IPC',
  'CLAUDE_CONFIG_DIR',
]);
...
function isDangerousEnvKey(key: string): boolean {
  return (
    ...
    key.startsWith('TINYCODE_SESSION_') ||
    key.startsWith('TINYCODE_INTERNAL_') ||
    key === 'TINYCODE_HOST_IDENTITY_MODE' ||
    key === 'TINYCODE_HOST_UID' ||
    key === 'TINYCODE_HOST_GID' ||
    key === 'TINYCODE_PASSWD_FILE' ||
    key === 'TINYCODE_RECONCILE_SESSION_PERMISSIONS' ||
    key === 'TINYCODE_MOUNT_PREPARE_MODE' ||
    key === 'TINYCODE_RUNTIME_USER'
  );
}
...
function fromStoredProfile(
  ...
  return {
    ...
    anthropicModel: normalizeModel(
      stored.anthropicModel ?? (stored as any).tinycodeModel ?? '',
    ),
...
export function getContainerEnvConfig(folder: string): ContainerEnvConfig {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      ) as ContainerEnvConfig & { tinycodeModel?: string };
      // Backward compat: migrate old field name
      if (
        stored.anthropicModel === undefined &&
        stored.tinycodeModel !== undefined
      ) {
        stored.anthropicModel = stored.tinycodeModel;
        delete stored.tinycodeModel;
      }
      return stored;
    }
  } catch (err) {
